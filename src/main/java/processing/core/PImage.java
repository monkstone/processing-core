...
public class PImage implements PConstants, Cloneable {
     ...
     * the <b>width</b> and <b>height</b> of the image, as well as an array
     * called <b>pixels[]</b> which contains the values for every pixel in the
     * image. A group of methods, described below, allow easy access to the
    ...
    /**
     * @param width
     * @param height public PImage(int width, int height, int format, int
     * factor) { init(width, height, format, factor); }
     *
     * /**
     ...
     * that you've done the work of making sure a MediaTracker has been used to
     * fully download the data and that the img is valid.
     *
     * @nowebref
     * @param img assumes a MediaTracker has been used to fully download the
     * data and the img is valid
     */
     ...
     * the array, there's no need to call <b>updatePixels()</b>. renderers may
     * or may not seem to require <b>loadPixels()</b>
     * or <b>updatePixels()</b>. However, the rule is that any time you want to
     * manipulate the <b>pixels[]</b> array, you must first call
     * <b>loadPixels()</b>, and after changes have been made, call
     * <b>updatePixels()</b>. Even if the renderer may not seem to use this
     ...
     * ( end auto-generated )
     * <h3>Advanced</h3>
     * Mark the pixels in this region as needing an update. This is not
     * currently used by any of the renderers, however the api is structured
     ...
     * do this consistently across renderers: technically infeasible with P3D,
     * or what would it even do with PDF?) If you want to resize PGraphics
 ...
 parameters are specified, the entire image is returned. Use the <b>x</b>
     ...
     * for the <b>blend()</b> function. ( end auto-generated )
     * <h3>Advanced</h3>
     * <UL>
...
int testFunction(int dst, int src) {
   ...
   * O = DS
     ...
     *
     * O = 2 * MULTIPLY(D, S) = 2DS for S < 0.5 O = 2 * SCREEN(D, S) - 1 = 2(S +
     * D - DS) - 1 otherwise
     */
    ...
    /**
     * Soft Light (Pegtop) O = (1 - D) * MULTIPLY(D, S) + D * SCREEN(D, S) O =
     * (1 - D) * DS + D * (1 - (1 - D)(1 - S)) O = 2DS + DD - 2DDS
     */
     ...
     * filename, the image will save in TIFF format and .tif will be added to
     * the name. These files are saved to the sketch's folder, which may be
     * opened by selecting "Show sketch folder" from the "Sketch" menu. To save
     * an image created within the code, rather than through loading, it's
     * necessary to make the image with the
